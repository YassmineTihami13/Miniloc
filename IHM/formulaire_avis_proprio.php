...
$stmt_check = $conn->prepare("SELECT COUNT(*) FROM evaluation WHERE reservation_id = :res_id AND evaluateur_id = :proprio_id");
$stmt_check->execute([
...
if ($evaluation_exists) {
    ...
    try {
        ...
        // Insertion de l'évaluation dans la table evaluation
        $stmt = $conn->prepare("INSERT INTO evaluation
            (objet_id, evaluateur_id, evalue_id, note, commentaire, date, reservation_id)
            VALUES (:objet_id, :proprio_id, :client_id, :note, :comment, NOW(), :res_id)");
